...
class EmployeeCMS {
  ...
  start() {
    figlet("Employee CMS", (err, data) => console.log(err || data));
  ...
  viewData(table) {
    let query = "";
    // switch on table to select query for combined data view
    switch (table) {
      case "departments":
  ...
  updateDBTableRow(table, setData) {
    ...
    this.dbConn
      .query(query, setData)
      .then(() => console.log(`Successfully updated date in ${table} table\n`))
      .catch((error) => console.log(error))
      .then(() => this.menu());
  }
  ...
  async promptDataEntry(data) {
    let dataInput;
    // switch on data for table in which to add new entry
    switch (data) {
      case "department":
