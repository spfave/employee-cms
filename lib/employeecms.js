...
class EmployeeCMS {
  ...
  async promptDataEntry(data) {
    ...
    switch (data) {
      case "department":
        dataInput = await inquirer.prompt(Department.prompts);
        this.addDBTableRow("departments", dataInput);
        break;

      case "role":
        const departmentsWithID = await this.getDataWithID("departments");
        dataInput = await inquirer.prompt(Role.prompts(departmentsWithID));
      ...
      case "employee":
        ...
        dataInput = await inquirer.prompt(
          Employee.prompts(rolesWithID, employeesWithID)
        );
  ...
  async getDataWithID(data) {
    ...
    return rows.map(({ dataWithID }) => dataWithID);
  }
}
